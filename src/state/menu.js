var menuState = {
	...
	create: function () {
		...
		for(i=0; i<KT.highscore.length; i++){
			KT.total_highscore += KT.highscore[i].highscore;
		}
		console.log(KT.total_highscore);
		...
		KT.grap.events.onDragStop.add(() => {
			if (KT.grap.position.y > KT.game.height / 4) {
				var tweenGrap = KT.game.add.tween(KT.grap).to({ y: KT.game.height / 4 }, 250, "Linear");
				tweenGrap.start();
			}
			if (KT.grap.position.y < -(KT.tabsSongHeight - KT.tabsSongHeight / 15 * 4 - KT.game.height / 4)) {
				var tweenGrap = KT.game.add.tween(KT.grap).to({ y: -(KT.tabsSongHeight - KT.tabsSongHeight / 15 * 4 - KT.game.height / 4) }, 250, "Linear");
				tweenGrap.start();
			}
		});
		KT.tabsSongHeight = 0;
	...
	createTabSong: function (i, songData) {
		...
		if (songData.status == "unlocked") {
			...
			if (songInHighscore.perfectPercent > 30 && songInHighscore.perfectPercent <= 60) {
				var starList = [1, 0, 0];
			} else if (songInHighscore.perfectPercent > 60 && songInHighscore.perfectPercent <= 80) {
				var starList = [1, 1, 0];
			} else if (songInHighscore.perfectPercent > 80) {
				var starList = [1, 1, 1];
			} else {
				var starList = [0, 0, 0];
			}
		...
		KT.tabsSongHeight += tabSong.height;
	},
	chooseSong: function (greenNotes, orangeNotes, violetNotes, x2Notes, timeDuration, nameOfSong, songData) {
		for (i = 0; i < greenNotes.length; i++) {
			KT.greenNotes.push(greenNotes[i]);
		}
		for (i = 0; i < orangeNotes.length; i++) {
			KT.orangeNotes.push(orangeNotes[i]);
		}
		for (i = 0; i < violetNotes.length; i++) {
			KT.violetNotes.push(violetNotes[i]);
		}
		for (i = 0; i < x2Notes.length; i++) {
